...
ImGui:Connect(function(ImGui)
	...
	local widgetDemos = {
		Basic = function()
			...
			do
				...
				local radioButtonState = ImGui.State(1)
				ImGui.Button({ "Button" })
		...
		Image = function()
			...
			do
				...
				local PixelatedState = ImGui.ComputedState(PixelatedCheckState, function(check)
	...
	do
		styleEditor = function()
			local styleList = {
				{
					...
					function()
						...
						local function SliderInput(input, arguments)
				...
				},
				{
					...
					function()
						...
						for name, font in next, fonts do
			...
			do
				...
				ImGui.TabBar()
				do
					for i, v in next, ipairs(styleList) do
						ImGui.Tab({ v[1] })
						do
							styleList[i][2]()
						end
	...
	-- main demo window
	return function()
		local NoTitleBar = ImGui.State(false)
		local NoBackground = ImGui.State(false)
		local NoCollapse = ImGui.State(false)
		local NoClose = ImGui.State(true)
		local NoMove = ImGui.State(false)
		local NoScrollbar = ImGui.State(false)
		local NoResize = ImGui.State(false)
		local NoNav = ImGui.State(false)
		local NoMenu = ImGui.State(false)
